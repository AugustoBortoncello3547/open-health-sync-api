...
export class CreateDadoSaudePacienteController implements ICreateDadoSaudePacienteController {
  ...
  async handle(
    ...
    const dadoSaudePacienteCreated = await this.getDadoSaudePacienteRepository.getDadoSaudePaciente(
      idRegistro,
      idAplicacao,
      idPaciente,
    );
    if (dadoSaudePacienteCreated) {
      await this.dispatchEventController.dispatch(
        idAplicacao,
        paciente.idAmbiente,
        EventTypeDispatchEnum.CREATE_DADO_SAUDE_PACIENTE,
        dadoSaudePacienteCreated,
      );
    }
